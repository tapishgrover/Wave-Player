      ...
      ZStack {
        VStack {
          ...
          HStack {
            ...
            // Debug badge – shows "EQ" and whether local file is detected
            if viewModel._playFromLocal {
                HStack(spacing: 4) {
                    Text("EQ")
                    Text(viewModel._playFromLocal ? "Y" : "N") // should always be Y here
                        .font(.system(size: 10))
                }
                .font(.caption2)
                .padding(4)
...
// (Keep all the existing AudioSettings views – GraphicEqualizerView, EqualLoudnessView, etc. – they are unchanged)
